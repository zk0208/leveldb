...
SingleTree::SingleTree(DBImpl *db, uint32_t id)
  ...
  versions_(new VersionSet(db_->dbname_, &db_->options_, db_->table_cache_, &db_->internal_comparator_, id_)){}
...
Status SingleTree::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  MutexLock l(&mutex_); //就是上锁而已，实现自动析构时解锁
  writers_.push_back(&w);
  //多线程写入时，当处于队首线程阻塞时，其他线程就算获得线程，也得等待，且把锁释放出去
  ...
  if (status.ok() && updates != nullptr) {  // nullptr batch is for compactions
    ...
    // into mem_.
    {
      mutex_.Unlock();
      db_->mutex_.Lock();
      status = db_->log_->AddRecord(WriteBatchInternal::Contents(updates));
  ...
  while (true) {
    if (!bg_error_.ok()) {
      ...
      mutex_.Lock();
               ...
               (mem_->ApproximateMemoryUsage() <= db_->options_.write_buffer_size)) {  //有空间
...
Status SingleTree::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  Log(db_->options_.info_log, "SingleTree %u, Level-0 table #%llu: started",id_,
...
void SingleTree::BackgroundCompaction() {
  ...
  if (c == nullptr) {
    // Nothing to do
  } else if (!is_manual && c->IsTrivialMove()) {  //不需要合并，只需要简单的移动
...
Status SingleTree::DoCompactionWork(CompactionState* compact) {
  ...
  for (; input->Valid() && !db_->shutting_down_.load(std::memory_order_acquire);) {
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (!has_current_user_key ||
          db_->user_comparator()->Compare(ikey.user_key, Slice(current_user_key)) !=
              0) {
...
//目前的实现时WAL 暂时不处理，然后info_log、block cache、table cache保持整个DB一个
//可配置参数仍在options.h中，不可改变配置在dbformat.h中
...
Iterator* DBImpl::NewInternalIterator(const ReadOptions& options,
  ...
  *latest_snapshot = singleTrees_[0]->versions_->LastSequence();
  ...
  // Collect together all singletree iterators
  std::vector<Iterator*> list;
...
void DBImpl::RecordReadSample(Slice key) {
  ...
  ParsedInternalKey ikey;
  if(ParseInternalKey(key, &ikey)) {
    uint32_t Tree_id = GetSingleTreeID(ikey.user_key.data(), ikey.user_key.size()) % config::kNumSingleTrees;
...
void DBImpl::GetApproximateSizes(const Range* range, int n, uint64_t* sizes) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (uint32_t j = 0; j < config::kNumSingleTrees; j++) {
      uint64_t size =0;
      singleTrees_[i]->GetApproximateSizes(&range[i], 1, &size);
      sizes[i] += size;
    }
...
Status DBImpl::Recover(VersionEdit* edit, bool* save_manifest) {
  ...
  /***以下是恢复log文件，暂时实现每个tree一个，按照每个version里面的log值中最小的一个来进行恢复***/
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    if (ParseFileName(filenames[i], &number, &type)) {
      ...
      if (type == kLogFile && ((number >= min_log) || (std::find(prev_logs.begin(), prev_logs.end(), number)!=prev_logs.end())))
      ...
      switch (type) {
        case kLogFile:
          keep = ((number >= min_log_num) ||
                  (std::find(prev_logs.begin(), prev_logs.end(), number)!=prev_logs.end()));
          break;
        case kDescriptorFile:
