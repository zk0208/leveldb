...
do
    ...
    # read
    workload="read_40Threads_pagecache_singleVol"
    ...
    free -h >> ${resultFile}
    ../build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --threads=40 --use_existing_db=1 --reads=524288 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${resultFile} #2>&1
    free -h >> ${resultFile}
    pidof iostat | xargs kill -9
...
do
    # 复制已有数据
    cp -r /home/colin/hub/testBackup/100G/origin_normal_wtCnums_100G_2021-12-16_09:22:34/* /home/colin/hub/testDir
    sudo bash -c "echo 1 > /proc/sys/vm/drop_caches"
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${resultFile}
    free -h >> ${resultFile}
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --threads=40 --use_existing_db=1 --reads=524288 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${resultFile} #2>&1
    pidof iostat | xargs kill -9
    free -h >> ${resultFile}
    echo "origin readrandom 40Threads end!!!"
    sudo bash -c "echo 1 > /proc/sys/vm/drop_caches"
