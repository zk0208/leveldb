...
#hotread
    ...
    workload="hotread_30Threads_pagecache"
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/leveldb/build/db_bench --benchmarks="readhot,stats,sstables" --db="/home/colin/hub/testDir" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
...
#hotread
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/my-leveldb/build/db_bench --benchmarks="readhot,stats,sstables" --db="/home/colin/hub/testDir" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
...
#hotread
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd /dev/md0 > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="readhot,stats,sstables" --db="/home/colin/hub/testDir_raid0" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
