...
do
    ...
    /home/colin/hub/my-leveldb/build/db_bench --benchmarks="fillrandom,stats,sstables" --db="/home/colin/hub/testDir" --num=62914560 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
...
do
    ...
    lsblk >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    ../build/db_bench --benchmarks="fillrandom,stats,sstables" --db="/home/colin/hub/testDir" --num=62914560 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
    pidof iostat | xargs kill -9
    # if [ ${loop} == 1 ]
    #     then 
...
do
    ...
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="fillrandom,stats,sstables" --db="/home/colin/hub/testDir_raid0" --num=62914560 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
    pidof iostat | xargs kill -9
    # if [ ${loop} == 1 ]
    #     then 
