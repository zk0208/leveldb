...
# done
    ...
    workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="origin_normal_tmpfs"
    # load
    workload="load"
    # iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd /dev/md0 > ${style}IO_${workload}_${workTime}.txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="fillrandom,stats,sstables" --db="/home/colin/tmpfsTest" --num=62914560 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
