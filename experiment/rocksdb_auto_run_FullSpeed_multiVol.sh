...
# done
    ...
    # # 后台4个compaction线程，看是否可以压满磁盘
    # workTime=`date +%Y-%m-%d_%H:%M:%S`
    # # 记录了compaction的数量和时延
    # style="origin_normal_singleVol_rocksdb_multiCompaction_6_multipath"
    # # load
    # workload="load"
    # iostat -mxt 1 3600 /dev/sdb /dev/sdc /dev/sdd> ${style}IO_${workload}_${workTime}.txt &
    # echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    # lsblk >> ${style}OUT_${workload}_${workTime}.txt
    # /home/colin/hub/rocksdb/build/db_bench_multi --benchmarks="fillrandom,stats" -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/testDir" -num=62914560 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    # echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    # # 中止iostat
...
do
    ...
    workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="origin_normal_multiVol_rocksdb_multiCompaction_6_multipath"
    # load
    workload="load"
    iostat -mxt 1 3600 /dev/sdb /dev/sdc /dev/sdd> ${style}IO_${workload}_${workTime}.txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    du -h /home/colin/hub/YCSB-C-RocksDB/build/testDir >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
    pidof iostat | xargs kill -9
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/rocksdb/build/db_bench_multi --benchmarks="overwrite,stats" -use_existing_db=true -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/testDir" -num=10485760 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    du -h /home/colin/hub/YCSB-C-RocksDB/build/testDir >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
    pidof iostat | xargs kill -9
    ...
    workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="origin_normal_multiVol_raid0_rocksdb_multiCompaction_6_multipath"
    # load
    workload="load"
    iostat -mxt 1 3600 /dev/sdb /dev/sdc /dev/sdd /dev/md0 > ${style}IO_${workload}_${workTime}.txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/rocksdb/build/db_bench_single --benchmarks="fillrandom,stats" -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/testDirRaid" -num=83886080 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/rocksdb/build/db_bench_single --benchmarks="overwrite,stats" -use_existing_db=true -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/testDirRaid" -num=10485760 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    du -h /home/colin/hub/YCSB-C-RocksDB/build/testDirRaid >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
    pidof iostat | xargs kill -9
