...
do
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    ../build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9

    # read
    workload="read_30Threads_directio"
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    ../build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --direct_io=1 --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${style}IO_${workload}_${workTime}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}\(${loop}\).txt
    ../build/db_bench --benchmarks="overwrite,stats,sstables" --db="/home/colin/hub/testDir" --use_existing_db=1 --num=10485760 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
...
#hotread
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    ../build/db_bench --benchmarks="readhot,stats,sstables" --db="/home/colin/hub/testDir" --threads=30 --use_existing_db=1 --reads=10485760 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 --direct_io=1 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
done 
# 删除数据
...
do
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/my-leveldb/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/my-leveldb/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir" --direct_io=1 --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
...
do
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd /dev/md0 > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir_raid0" --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
    ...
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd /dev/md0 > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir_raid0" --direct_io=1 --threads=30 --use_existing_db=1 --reads=349525 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    pidof iostat | xargs kill -9
