...
#         done
    ...
    # 挂在磁盘
    sudo mount /dev/sdb1 /home/colin/hub/testDir
    sudo mount /dev/sdc1 /home/colin/hub/testDir_raid0
...
do
    ...
    # read
    workload="read_singleThreads_pagecache_singleVol"
    ioFile=${workTime}_${style}IO_${workload}\(${loop}\).txt
    resultFile=${workTime}_${style}OUT_${workload}\(${loop}\).txt
    iostat -mx 1 3600 /dev/sdb > ${ioFile} &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${resultFile}
    free -h >> ${resultFile}
    ../build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir"  --use_existing_db=1 --reads=524288 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${resultFile} #2>&1
    free -h >> ${resultFile}
    pidof iostat | xargs kill -9

    echo "levelbase readrandom singleThreads end!!!"
    sudo bash -c "echo 1 > /proc/sys/vm/drop_caches"
...
do
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${resultFile}
    free -h >> ${resultFile}
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="readrandom,stats,sstables" --db="/home/colin/hub/testDir_raid0" --use_existing_db=1 --reads=524288 --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${resultFile} #2>&1
    pidof iostat | xargs kill -9
    free -h >> ${resultFile}
    echo "origin readrandom singleThreads end!!!"
    sudo bash -c "echo 1 > /proc/sys/vm/drop_caches"
