...
do
    ...
    style="blockbase_normal_wtCnums_100G_seq"
    ...
    /home/colin/hub/my-leveldb/build/db_bench --benchmarks="fillseq,stats,sstables" --db="/home/colin/hub/testDir" --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
    ...
    if [ ${loop} == 1 ]
        then 
            cp -r /home/colin/hub/testDir /home/colin/hub/testBackup/100GSEQ/${style}_${workTime}
    fi
    # 删除数据
...
do
    ...
    # workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="levelbase_normal_wtCnums_100G_seq"
    ...
    lsblk >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt
    iostat -mx 1 3600 /dev/sdb /dev/sdc /dev/sdd > ${workTime}_${style}IO_${workload}\(${loop}\).txt &
    ../build/db_bench --benchmarks="fillseq,stats,sstables" --db="/home/colin/hub/testDir" --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
    pidof iostat | xargs kill -9
    # if [ ${loop} == 1 ]
    #     then 
...
do
    ...
    style="origin_normal_wtCnums_100G_seq"
    ...
    /home/colin/hub/leveldb_origin/build/db_bench --benchmarks="fillseq,stats,sstables" --db="/home/colin/hub/testDir_raid0" --num=104857600 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${workTime}_${style}OUT_${workload}\(${loop}\).txt 2>&1
    # 中止iostat
    pidof iostat | xargs kill -9
    # if [ ${loop} == 1 ]
    #     then 
    ...
    if [ ${loop} == 1 ]
        then 
            cp -r /home/colin/hub/testDir_raid0 /home/colin/hub/testBackup/100GSEQ/${style}_${workTime}
    fi
