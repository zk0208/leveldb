...
# done
    ...
    workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="origin_normal_singleVol_google_leveldb"
    # load
    workload="load"
    iostat -mx 1 3600 /dev/sdb > ${style}IO_${workload}_${workTime}.txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/google_leveldb/build/db_bench --benchmarks="fillrandom,stats,sstables" --db="/home/colin/hub/YCSB-C-RocksDB/build/sdb" --num=62914560 --value_size=1000 --write_buffer_size=67108864 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
