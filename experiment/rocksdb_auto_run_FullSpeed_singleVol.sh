...
# done
    ...
    # workTime=`date +%Y-%m-%d_%H:%M:%S`
    # # 记录了compaction的数量和时延
    # style="origin_normal_singleVol_rocksdb_multiCompaction_6"
    # # load
    # workload="load"
    ...
    # /home/colin/hub/rocksdb/build/db_bench_single --benchmarks="fillrandom,stats" -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/sdb" -num=62914560 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    # echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    # # 中止iostat
...
#卸载 raid0
    ...
    sudo mount /dev/sdc1 /home/colin/hub/YCSB-C-RocksDB/build/sdc
    sudo chown colin:colin /home/colin/hub/YCSB-C-RocksDB/build/sdc
 ...
 # 后台4个compaction线程，看是否可以压满磁盘
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/rocksdb/build/db_bench_single --benchmarks="fillrandom,stats" -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/sdc" -num=83886080 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    du -h /home/colin/hub/YCSB-C-RocksDB/build/sdc >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
    pidof iostat | xargs kill -9
    ...
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/rocksdb/build/db_bench_single --benchmarks="overwrite,stats" -use_existing_db=true -compression_type=none -db="/home/colin/hub/YCSB-C-RocksDB/build/sdc" -num=10485760 -value_size=1000 -max_background_jobs=6 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    du -h /home/colin/hub/YCSB-C-RocksDB/build/sdc >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
    pidof iostat | xargs kill -9
