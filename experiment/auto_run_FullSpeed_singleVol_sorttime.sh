...
# done
    ...
    workTime=`date +%Y-%m-%d_%H:%M:%S`
    # 记录了compaction的数量和时延
    style="origin_normal_singleVol_sorttime"
    # load
    workload="load"
    iostat -mxt 1 3600 /dev/sdg > ${style}IO_${workload}_${workTime}.txt &
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    lsblk >> ${style}OUT_${workload}_${workTime}.txt
    /home/colin/hub/leveldb_origin/build/db_bench_sorttime --benchmarks="fillrandom,stats,sstables" --db="/home/colin/hub/YCSB-C-RocksDB/build/backup/testDir" --num=62914560 --value_size=1000 --write_buffer_size=67108864 --max_file_size=67108864 >> ${style}OUT_${workload}_${workTime}.txt 2>&1
    echo `date +%Y-%m-%d_%H:%M:%S` >> ${style}OUT_${workload}_${workTime}.txt
    # 中止iostat
